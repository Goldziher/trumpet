...
//!    heartbeat within `agents.timeout_secs`.
...
//! 3. Fails tasks whose [`Task::deadline`] has passed (when set).
//!
//! The watchdog is intentionally simple — one task, one tick interval,
//! linear scans. Trumpet single-host workloads stay well below the
//! threshold where a smarter index would matter.
...
use crate::core::task_types::{MessageRole, Part, TaskFilter, TaskId, TaskMessage, TaskState};
use crate::core::types::{AgentStatus, MessageId};
...
/// transitioned to [`TaskState::Failed`] in the same pass.
pub fn spawn(
